...
const asrCorrections = [
  ...
  { term: "ReAct", aliases: ["Re Act", "瑞艾克特"] },
  { term: "Harness", aliases: ["哈尼斯", "哈内斯"] },
  // AI 编程工具常以英文发音出现，ASR 往往保留了近似拼写；统一后才能命中术语表和产品分析题。
  { term: "Cursor", aliases: ["COSER", "Coser", "卡瑟", "卡色", "卡索"] },
  { term: "Qoder", aliases: ["quder", "Quder", "扣德儿", "扣德尔", "库德儿", "库德尔", "扣德", "库德", "Q coder"] },
  { term: "Work Buddy", aliases: ["WorkBuddy", "work buddy", "workbuddy", "沃克巴迪", "沃克巴弟", "沃克巴蒂", "沃克巴蒂"] },
...
function normalizeAsrQuestionNoise(question = "") {
  ...
  // ASR 常会把“请介绍一下”重复拼接，并在中间插入孤立英文字母。
  // 只在“介绍 + 已识别项目名”的明确语境中收敛，避免误删真实产品名或技术缩写。
  if (/(?:介绍|讲讲|说说).{0,24}(?:GEO|旅游智能营销|Lai\s*trip)/iu.test(normalized)) {
    if (/(?:请?介绍){2,}.{0,24}(?:GEO|旅游智能营销|Lai\s*trip)/iu.test(normalized)
      || /请介绍(?:一下)?[a-z]?介绍一下\s*GEO/iu.test(normalized)) {
      const project = /旅游智能营销|Lai\s*trip/iu.test(normalized) ? "旅游智能营销" : "GEO";
      normalized = `请介绍一下 ${project} 这个项目。`;
    }
  }
