...
import { prepareQuestionForSubmission } from "./src/question-turn.js";
...
import { createLocalBackup, parseLocalBackup } from "./src/local-backup.js";
import { nextOverlayWindowMode } from "./src/overlay-mode-sync.js";
import { collectOverlayInteractiveRegions } from "./src/overlay-interactive-regions.js";
import { resolveAnswerOverlayPresentation } from "./src/answer-overlay-state.js";
...
function currentBackupGlossary() {
  return state.glossaryMarkdown?.trim() ? { name: state.glossaryFileName, markdown: state.glossaryMarkdown } : null;
}

function currentBackupRules() {
  return state.answerRules?.name && state.answerRules?.markdown?.trim() ? { name: state.answerRules.name, markdown: state.answerRules.markdown } : null;
}

function exportLocalBackup() {
  const backup = createLocalBackup({
    documents: documentPayload(),
    glossary: currentBackupGlossary(),
    rules: currentBackupRules(),
    preferences: { questionCaptureHotkey: state.questionHotkey, activeProjectId: state.activeProjectId },
  });
  const date = new Date().toISOString().slice(0, 10);
  downloadTextFile({ name: `面试资料伴侣备份-${date}.json`, text: `${JSON.stringify(backup, null, 2)}\n` });
}

async function persistImportedRules(rules) {
  const response = await fetch("/api/rules", {
    method: rules ? "PUT" : "DELETE",
    headers: rules ? { "content-type": "application/json" } : undefined,
    body: rules ? JSON.stringify(rules) : undefined,
  });
  const payload = await response.json();
  if (!response.ok) throw new Error(payload.error || "回答规则保存失败");
...
async function importLocalBackupFile(file) {
  ...
  let backup;
  try {
    backup = parseLocalBackup(JSON.parse(await file.text()));
    if (backup.glossary && !parseGlossaryMarkdown(backup.glossary.markdown).length) throw new Error("备份术语表没有有效术语");
  } catch (error) {
    window.alert(error.message || "备份文件无效");
    return;
  }
  if (!window.confirm("导入会替换当前资料、术语表、回答规则和快捷键；不会覆盖 API Key、Access Token 或语音服务配置。是否继续？")) return;
  try {
    state.documents = await hydrateDocumentSections(backup.documents);
    state.deletedDocuments = [];
    refreshSearchSections();
    const activeSkill = [...state.documents].reverse().find((document) => document.type === "skill");
    state.template = activeSkill?.markdown || defaultTemplate;
    state.templateName = activeSkill?.name || "面试口头回答模板";
    state.activeProjectId = backup.preferences.activeProjectId || "";
    state.questionHotkey = backup.preferences.questionCaptureHotkey || "Alt+Space";
    if (backup.glossary) {
      state.glossary = mergeGlossaryTerms(parseGlossaryMarkdown(backup.glossary.markdown));
      state.glossaryFileName = backup.glossary.name;
      state.glossaryMarkdown = backup.glossary.markdown;
    } else {
    ...
    writeStorage("interview.template", state.template);
    writeStorage("interview.templateName", state.templateName);
    writeStorage("interview.deletedDocuments", JSON.stringify([]));
    writeStorage("interview.activeProjectId", state.activeProjectId);
    writeStorage("interview.questionCaptureHotkey", state.questionHotkey);
    await persistDocuments();
    persistGlossary();
    await persistImportedRules(backup.rules);
    ...
    await syncQuestionCaptureHotwords();
    updateQuestionCaptureHotkeyUi();
    updateSkillPreview();
    renderDocuments();
    renderRulesSettings();
    window.alert("备份已导入。资料、术语表、回答规则和快捷键已恢复；API 配置保持不变。");
  } catch (error) {
    window.alert(error.message || "备份导入失败，请检查本机服务后重试");
  }
}
...
function renderAnswerOverlay() {
  ...
  const presentation = resolveAnswerOverlayPresentation({
    expandedRequested: state.answerOverlayExpanded,
    compactRequested: state.answerOverlayCompact,
    hasCurrentAnswer: Boolean(answerState.current),
  });
  const { expanded, compact } = presentation;
  overlay.classList.toggle("expanded", expanded);
  overlay.classList.toggle("compact", compact);
  $("answerOverlayBackdrop").classList.toggle("hidden", !expanded || compact);
  ...
  const compactButton = $("overlayCompactButton");
  // 这不是“展开回答”的上/下箭头：它只负责隐藏或恢复整条工具栏。
  // 用面板图标避免和 answerOverlayToggle 的箭头产生歧义。
  compactButton.innerHTML = `<i data-lucide="${compact ? "panel-top-open" : "panel-top-close"}"></i>`;
  compactButton.setAttribute("aria-label", compact ? "恢复工具栏" : "隐藏工具栏");
  compactButton.title = compact ? "恢复工具栏" : "隐藏工具栏";
  ...
  syncOverlayWindow(presentation);
}

function syncOverlayWindow(presentation = resolveAnswerOverlayPresentation({
  expandedRequested: state.answerOverlayExpanded,
  compactRequested: state.answerOverlayCompact,
  hasCurrentAnswer: Boolean(answerState.current),
})) {
  ...
  // 设置页和已展开回答都是完整、可交互的原生窗口；不需要再登记悬浮工具条
  // 的透明热区。此前这里仍连续发三次 IPC，会让“设置”看起来要等很久才打开。
  if (mode === "settings" || mode === "expanded") return;
  const generation = ++overlayRegionSyncGeneration;
  const syncRegions = () => {
    if (generation !== overlayRegionSyncGeneration) return;
    // 收起态需要把整条可见工具栏登记为原生可交互区：它同时是窗口拖动区。
    // 紧凑态则只保留恢复按钮，绝不留下旧工具栏的隐形热区。
    const toolbar = $("answerOverlay")?.querySelector(".question-toolbar");
    const controls = presentation.compact
    ...
    void window.interviewApp?.setOverlayInteractiveRegions?.(collectOverlayInteractiveRegions(controls));
  };
  // 原生窗口会在 180ms 内从紧凑态过渡到完整工具栏；只在第一帧取一次
  // DOM 坐标会留下 56px 状态的旧热区，导致恢复后点不到或挡住桌面。
  window.requestAnimationFrame(syncRegions);
  window.setTimeout(syncRegions, 80);
  window.setTimeout(syncRegions, 220);
}
...
function toggleAnswerOverlay() {
  if (state.answerOverlayCompact) {
    state.answerOverlayCompact = false;
    state.answerOverlayExpanded = Boolean(answerState.current);
  } else if (!answerState.current) {
...
function toggleOverlayCompact() {
  // 紧凑态只代表收起工具栏。若当前正展示答案，先关闭答案主体，
  // 否则恢复时会带着透明的大窗口覆盖桌面并截获外部点击。
  // 恢复必须使用普通 click：pointerdown 与 macOS 原生拖拽区域竞争，
  // 会导致需要多次点击或恢复后仍沿用 56px 热区。
  if (!state.answerOverlayCompact) state.answerOverlayExpanded = false;
  state.answerOverlayCompact = !state.answerOverlayCompact;
  renderAnswerState();
}

function startQuestionCaptureFromHotkey() {
  // 全局快捷键是主动开始下一题的入口；紧凑态下先立即恢复工具栏，
  // 然后不等待窗口动画，直接启动识别，避免首个语音片段丢失。
  if (state.answerOverlayCompact) {
    state.answerOverlayCompact = false;
    renderAnswerState();
  }
  void startRepeatQuestion({ source: "hotkey" });
}
...
function setupOverlayWindowDrag() {
  ...
  const compactButton = $("overlayCompactButton");
  const toolbar = $("answerOverlay")?.querySelector(".question-toolbar");
  // 紧凑态唯一按钮同时承载“轻点恢复”和“拖动位置”。触控板/鼠标的普通
  // 点击常会带有 2～4px 的抖动；阈值过小会把点击错判成拖动并吞掉 click。
  const dragStartDistance = 8;
  let dragStart = null;
  let moved = false;
  // 紧凑按钮用 pointer 手势直接判定“点击”或“拖动”。不要把是否展开交给
  // 浏览器随后补发的 click：拖动松手也可能带来一个 click，造成误展开。
  let compactPointerAction = false;
  let compactPointerActionTimer = null;

  function markCompactPointerAction() {
    compactPointerAction = true;
    clearTimeout(compactPointerActionTimer);
    // 正常情况下合成 click 会立刻到达；超时只防止异常情况下标记滞留。
    compactPointerActionTimer = setTimeout(() => { compactPointerAction = false; }, 1200);
  }

  function clearCompactPointerAction() {
    compactPointerAction = false;
    clearTimeout(compactPointerActionTimer);
    compactPointerActionTimer = null;
  }

  function consumeCompactButtonClick(event) {
    if (compactPointerAction) {
      clearCompactPointerAction();
      event.preventDefault();
  ...
  function finishDrag(event) {
    ...
    const { pointerId, target } = dragStart;
    const didMove = moved;
    dragStart = null;
    moved = false;
    target.releasePointerCapture?.(pointerId);
    // 紧凑态的整块工具栏都是可见外壳；不能只把中间 40px 按钮视为点击目标，
    // 否则点到外壳边缘会落进普通拖动分支，造成“点击无反应”。
    const compactButtonGesture = state.answerOverlayCompact && (target === compactButton || target === toolbar);
    // 紧凑态没有依赖原生 click：松手时立即做唯一判定。
    // 未移动 = 恢复；移动过 = 只保存拖动位置，绝不恢复。
    if (compactButtonGesture) {
      event?.preventDefault?.();
      event?.stopPropagation?.();
      // pointercancel 代表系统取消了这次手势（例如窗口失焦），不能当成点击恢复。
      if (event?.type !== "pointerup") {
        if (didMove) void window.interviewApp?.finishOverlayDrag?.("compact");
        return;
      }
      markCompactPointerAction();
      if (!didMove) toggleOverlayCompact();
      else void window.interviewApp?.finishOverlayDrag?.("compact");
      return;
    }
    if (!didMove) return;
    event?.preventDefault?.();
    void window.interviewApp?.finishOverlayDrag?.(state.answerOverlayCompact ? "compact" : (state.answerOverlayExpanded ? "expanded" : "collapsed"));
  }
  ...
  const startDrag = (event) => {
    ...
    // 紧凑态的可见外壳是 56px，真正的按钮只有中间 40px。
    // 外壳边缘也必须能拖动；但完整工具栏不能把所有控制按钮都变成拖动区。
    if (target === toolbar && !state.answerOverlayCompact) return;
    // button 的事件会冒泡到 toolbar，避免同一次按下被重复初始化。
    if (dragStart) return;
    // 新的一次真实按下开始后，旧的浏览器合成 click 已经没有意义。
    clearCompactPointerAction();
    // 紧凑按钮的点击由 pointerup 直接处理；先阻止浏览器再补发一次 click。
    if ((target === compactButton || target === toolbar) && state.answerOverlayCompact) event.preventDefault();
    if (target === card) {
      event.preventDefault();
      window.getSelection?.()?.removeAllRanges?.();
    }
    dragStart = { x: event.screenX, y: event.screenY, pointerId: event.pointerId, target };
    moved = false;
    target.setPointerCapture?.(event.pointerId);
  };
  const moveDrag = (event) => {
    if (!dragStart || event.pointerId !== dragStart.pointerId) return;
    // 只在 pointerup 结算手势。部分触控板会在松手前先发一个 buttons=0 的
    // pointermove；若在这里结算，浏览器随后补发的 click 会把拖动误当作恢复。
    if ((event.buttons & 1) === 0) return;
    const deltaX = event.screenX - dragStart.x;
    const deltaY = event.screenY - dragStart.y;
    if (Math.abs(deltaX) < dragStartDistance && Math.abs(deltaY) < dragStartDistance) return;
    moved = true;
    event.preventDefault();
    dragStart = { x: event.screenX, y: event.screenY, pointerId: event.pointerId, target: dragStart.target };
  ...
  compactButton.addEventListener("pointerdown", startDrag);
  compactButton.addEventListener("pointermove", moveDrag);
  compactButton.addEventListener("pointerup", finishDrag);
  compactButton.addEventListener("pointercancel", finishDrag);
  compactButton.addEventListener("selectstart", (event) => event.preventDefault());
  // 紧凑态没有其它控件：整块外壳都可拖，轻点中心按钮仍沿用原来的恢复 click。
  ...
  toolbar?.addEventListener("selectstart", (event) => event.preventDefault());
  window.addEventListener("pointerup", finishDrag, true);
  window.addEventListener("pointercancel", finishDrag, true);
  compactButton.addEventListener("click", consumeCompactButtonClick);
  toolbar?.addEventListener("click", (event) => {
    if (!state.answerOverlayCompact) return;
    // 点最右侧按钮从完整工具栏进入紧凑态时，这条 click 会继续冒泡到 toolbar。
    // 此时不能再执行一次恢复，否则状态会在同一帧“收起→展开”而表现为闪烁。
    if (event.target === compactButton || compactButton.contains(event.target)) return;
    consumeCompactButtonClick(event);
  });
...
function submitRepeatedQuestion() {
  ...
  const question = prepareQuestionForSubmission(state.repeatText);
...
function handleRepeatAsrEvent(payload) {
  ...
  // 原始转写只用于拼接分段；界面和后续提交始终使用同一份术语归一化文本，
  // 避免把“deepfake + seek harness”这类已可纠正的误识别展示或提交出去。
  const visibleTranscript = normalizeQuestion(state.repeatText, state.glossary);
  $("transcriptText").textContent = nextVisibleTranscript({
    current: $("transcriptText").textContent,
    incomingText: state.repeatAwaitingFinal ? "" : visibleTranscript,
    mergedText: visibleTranscript,
...
function setupModules() {
  ...
  $("overlaySettingsButton").addEventListener("click", () => openView("settingsView"));
  ...
  $("importLocalBackupButton").addEventListener("click", () => $("localBackupFileInput").click());
  $("localBackupFileInput").addEventListener("change", async (event) => {
    try { await importLocalBackupFile(event.target.files[0]); }
  ...
  if (window.interviewApp?.onQuestionCaptureHotkey) window.interviewApp.onQuestionCaptureHotkey(startQuestionCaptureFromHotkey);
  ...
  $("uploadConverterSkillButton").addEventListener("click", () => $("converterSkillFileInput").click());
...
function openView(viewId, settingsId = null) {
  ...
  document.body.classList.toggle("overlay-mode", viewId === "questionView");
