...
**回答：** AlphaRank GEO 是一个品牌 AI 认知监测与策略优化平台，核心不是批量生成营销内容，而是让企业看清品牌是否被 AI 提及、为什么推荐竞品、应该先改什么，以及改完是否有效。它面向品牌、市场和内容运营团队，把 AI Search 里的黑盒问题做成“监测—诊断—优化—验证”闭环：先用统一 Prompt Set 跨平台采集回答，再抽取品牌提及、推荐位置、引用来源和竞品差距，最后基于证据给出内容与信源优化动作并复测。现有材料能确认的个人职责，是我从产品侧参与梳理该闭环、归因流程、输入输出和判断标准；Prompt、Agent、RAG 和评测是平台方案上下文，具体算法与工程实现不归为我的个人交付。真实上线范围、付费收入和客户归属仍需单独核实，我不会把方案状态说成已规模化商业落地。
...
**关键词：** 品牌提及率、提及位置、推荐强度、引用权威、Visibility Score
**适用场景：** 指标体系、权重设计、数据口径追问
**回答：** 我会先区分单项指标和综合指标。品牌提及率是提及品牌的有效回答数除以有效回答总数；Visibility Score 则是品牌提及覆盖、提及位置、品牌推荐强度和引用权威四项在单个平台内的综合分，是项目的核心结果指标。跨平台总览时，我会按各平台有效回答样本量加权汇总，不把品牌提及率和 Visibility Score 混为同一个指标。

**背景：** 源材料同时出现了单项指标和综合指标；如果只报提及率，会遗漏推荐位置、推荐强度和引用质量，若不说明平台内计算和跨平台汇总，也无法比较不同平台与周期的结果。
**具体做法：**
1. 有效回答只包含成功且有有效正文的结果，超时、风控和空回答不进分母。
2. 品牌名、别名和中英文名先做实体归一，同一回答的品牌提及只记一次。
3. 分平台计算品牌提及率、提及位置、推荐强度、引用率与信源分布等单项指标，并保留竞品差距作为诊断信息。
4. 平台 Visibility Score 采用提及覆盖 45、提及位置 20、推荐强度 15、引用权威 20 的加总公式；跨平台总览再按有效回答样本量加权汇总。
5. 综合分权重、位置区间、推荐等级和信源等级都要版本化，并用人工标注集、样本分布和业务结果校准；内容准出阈值另按内容质量评分体系管理。

**结果：** 指标口径可计算、可下钻、可回看；45/20/15/20 是源材料给出的 V0.1 评分权重，仍需回测和业务结果校准，不能表述为已验证的最终算法。

**追问补充：**
- 平均提及排名只在已提及回答中计算，数值越小代表越靠前。
- 跨平台总览按各平台有效回答样本量加权汇总，避免默认将各平台等权处理。
...
2. 先按标题、段落和语义边界切片，再用 Token 上限和相邻重叠控制上下文；当前方案参数约 1200 Token、重叠 100 Token，需以最终配置为准。
...
**结果：** 方案覆盖了关键词与语义互补、租户隔离和引用回链。源材料中的未经验证数据“71.1% 提升到 85% 以上”缺少评测集、样本量和计算口径，确认前不作为项目结果。
...
### GEO 业务闭环图（原画板转写）
数据监测 → 发现问题 → 分析问题 → 生成方案 → 优化 → 验证效果；验证结果回流到数据监测，形成“持续优化”闭环。

### GEO 分层产品架构图（原画板转写）
- 应用层：GEO 报告生成（多平台分析、问题归因、情感分析、GEO 优化建议、品牌表现总览、竞品对比、品牌认知分析）、AI 解答助手（数据分析、策略分析、优化指引）、企业知识库（品牌档案、产品档案、权威文件）、项目管理（品牌项目切换、项目创建）、内容生产、提示词管理和数据看板。
- 能力层：Leader Agent 负责拆解任务、指派任务和汇总结果；Prompt Agent 负责资料库分析、Prompt 增强/扩展和 AI 问答数据汇总；数据 Agent 拆解/分析数据并将答案转换为指标；诊断 Agent 诊断页面和网页、输出评分、问题证据与优化建议；策略 Agent 识别内容/机会、划分优先级并规划建议；内容优化 Agent 按诊断结果修改已有网页或文章；内容生成 Agent 生成文章和图片。
- 模型层：大语言模型 Qwen3.5-5B-A3B、生图模型 Qwen Image 2.0 Pro、Embedding Qwen3-Embedding-4B、Rerank Qwen3-Reranker-4B。
- 数据层：业务主数据（项目、品牌、竞品、行业等）、监测数据（AI 回答、引用来源、排名、情感等）、RAG 知识库（文本切片、网页、品牌、行业、企业知识）、指标（GEO 指标、行业基准、诊断得分、趋势洞察、策略结果）。

### GEO 用户与交付流程图（原画板转写）
用户登录后经新手引导和全局预览了解 GEO；主业务流依次为 GEO 服务流程 → 意图挖掘 → 品牌诊断 → 知识库创建 → 创建/管理监控 → 生成内容 → 数据大屏，项目管理和题目管理提供横向支撑。交付泳道分产品、设计、研发、测试：需求澄清/方案设计，经各方评审后进入 UI/技术/测试用例设计与开发；评审不通过则回到对应设计环节；测试通过后联调、准备发布、发布上线并全量上线。

### GEO 数据采集与指标计算图（原画板转写）
Topic/Prompt 监测主题与问题 → 浏览器自动化采集 → 获取 AI Answer 与 Citation → 数据清洗 → 规则匹配与模型抽取 → 结构化字段。结构化字段一支汇总品牌提及、品牌排名和竞品信息，另一支汇总情感倾向和引用来源；数据聚合后生成 GEO 指标，计算可见度得分、品牌提及率、平均排名、引用率、品牌声量份额和情感倾向，并输出到基础/诊断/优化/监测等业务应用。
...
<sheet sheet-id="seMC1F" token="EiVbs7uyEhawXqtBRhNcx45enSk"></sheet>
...
<sheet sheet-id="oh1BHs" token="EiVbs7uyEhawXqtBRhNcx45enSk"></sheet>

#### 3.2 项目成本（待填入项目数据）

<sheet sheet-id="vKfSbg" token="EiVbs7uyEhawXqtBRhNcx45enSk"></sheet>
...
<whiteboard token="PCv1wA82xhpl6Dbh9CmcuDHpndf"></whiteboard>
...
<whiteboard token="XWN7wGAkWhtGQYbf6Djcgcp5n63"></whiteboard>

### 用户使用流程

<whiteboard token="EIFPwXNwlhlzp3bun3XcklkXnEh"></whiteboard>
...
<whiteboard token="WeXowPSj7hifG7b4TSFcVW8anZb"></whiteboard>
...
| --- | --- | --- | --- |
| 企业老板/负责人 | 采购者与最终决策者 | 希望以较低成本获得更多企业客户和咨询线索 | AI品牌体检、竞品排名、效果报告、线索变化 |
| 市场/运营负责人 | 核心高频用户 | 一个人完成品牌监测、内容规划和GEO优化 | 提问词挖掘、可见度监测、竞品分析、内容建议、效果复测 |
| 销售负责人 | 业务需求提出者 | 了解客户关心的问题及企业未覆盖的选型场景 | 用户问题洞察、产品对比场景、销售内容建议 |
| 内容/新媒体人员 | 优化执行者 | 根据客户需求生产产品介绍、案例、FAQ等内容 | 内容缺口、选题建议、内容优化、引用追踪 |
