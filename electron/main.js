...
import { normalizeOverlayPlacement, resolveCompactRestorePlacement, resolveOverlayDragFinish, resolveOverlayPlacement } from "../src/overlay-placement.js";
...
let overlayLastFullPosition = null;
...
function refreshOverlayMousePassthrough() {
  ...
  update();
  if ((overlayMode === "collapsed" || overlayMode === "compact") && overlayInteractiveRegions.length) overlayMousePassthroughTimer = setInterval(update, 40);
}
...
function loadOverlayPlacement() {
  try {
    ...
    overlayLastFullPosition = saved.position;
  } catch {}
}
...
function setOverlayWindowMode(mode) {
  ...
  overlayMode = mode;
  // 紧凑态只剩一个恢复/拖动按钮，绝不能继承收起态的透明区域鼠标穿透。
  if (mode === "compact") {
    if (overlayMousePassthroughTimer) clearInterval(overlayMousePassthroughTimer);
    overlayMousePassthroughTimer = null;
    setOverlayMouseIgnored(false);
  } else if (mode === "collapsed") {
    // 从 56px 紧凑窗恢复时，渲染层需要等原生窗口真正变宽后才能得到
    // 正确的按钮坐标。过渡期先全部穿透，不能沿用旧的紧凑态热区挡住桌面。
    if (overlayMousePassthroughTimer) clearInterval(overlayMousePassthroughTimer);
    overlayMousePassthroughTimer = null;
    // 只有真正从别的模式切回来时才清空旧热区；同一收起态的二次确认
    // 不能把已登记的工具栏热区删掉，否则会在每次 UI 重绘后挡住桌面。
    if (modeChanged) overlayInteractiveRegions = [];
    setOverlayMouseIgnored(true);
  } else {
    // 设置页和答案展开页是完整交互界面，不能保留收起态的鼠标穿透。
  ...
  if (mode === "compact" && previousMode !== "compact") {
    // 紧凑态的左上角不是完整工具栏的保存位置；单独保存完整态坐标。
    overlayLastFullPosition = { x: initialBounds.x, y: initialBounds.y };
  }
  ...
  const restoreFromCompact = previousMode === "compact" && mode !== "compact";
  const position = restoreFromCompact
    ? overlayLastFullPosition || resolveCompactRestorePlacement({ compactBounds: initialBounds, anchor: overlayAnchor, size, workArea })
    : overlayAnchor === "bottom" && mode === "expanded"
  ...
  const targetBounds = mode === "compact"
    ? { x: initialBounds.x + initialBounds.width - size.width, y: initialBounds.y, ...size }
    : { ...targetPosition, ...size };
  if (overlayWindowResizeTimer) clearInterval(overlayWindowResizeTimer);
  windowRef.setMinimumSize(mode === "compact" ? size.width : 460, mode === "compact" ? size.height : 52);
  // 紧凑态进出都必须一次确定地切换原生窗口，不能让 760px ↔ 56px 的动画
  // 与渲染层热区登记竞态。竞态会造成快速连点后只剩孤立按钮或点不到恢复按钮。
  // 收起必须立即缩到工具栏高度。若继续参与 180ms 动画，DOM 已经隐藏
  // 回答区时原生窗口仍是展开高度，会形成用户截图里的透明点击遮罩。
  if (mode === "settings" || previousMode === "compact" || mode === "compact" || mode === "collapsed") {
    windowRef.setBounds(targetBounds);
    // 进入紧凑态不覆盖完整工具栏位置；否则下一次恢复会跳到紧凑按钮坐标。
    if (mode !== "compact") overlayPosition = { x: targetBounds.x, y: targetBounds.y };
  ...
  const animate = () => {
    ...
    windowRef.setBounds({
      x: Math.round(initialBounds.x + (targetBounds.x - initialBounds.x) * eased),
      y: Math.round(initialBounds.y + (targetBounds.y - initialBounds.y) * eased),
      width: Math.round(initialBounds.width + (targetBounds.width - initialBounds.width) * eased),
    ...
    if (progress === 1 && overlayWindowResizeTimer) {
      ...
      if (mode !== "compact" && !(mode === "expanded" && overlayAnchor === "bottom")) overlayPosition = { x: targetBounds.x, y: targetBounds.y };
      saveOverlayPlacement();
    }
  };
  animate();
...
async function createWindow() {
  ...
  // 紧凑态由渲染层的 pointer drag 驱动。不要监听 moved 后再次吸附：
  // setPosition 本身也会触发 moved，曾导致一次拖动被第二条链路抢走。
...
ipcMain.handle("window:move-overlay-by", (_event, deltaX, deltaY) => {
  ...
  if (overlayMode !== "compact") overlayLastFullPosition = overlayPosition;
  overlayAnchor = "free";
  return true;
});
ipcMain.handle("window:finish-overlay-drag", (_event, mode = "collapsed") => {
  ...
  overlayLastFullPosition = placement.position;
  saveOverlayPlacement();
